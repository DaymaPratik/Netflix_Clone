...
function  TvsSlider({singleCategory}) {
    const [topRatedMoviesArr,setTopRatedMoviesArr]=useState([]);
    ...
    useEffect(()=>{
        const getTopRatedMoviesFunction=async()=>{
            try {
                const response=await fetch(`https://netflix-clone-ghjh.onrender.com/api/tv/${singleCategory}`,{
                  method:"GET",
                  headers:{
                ...
                setTopRatedMoviesArr(data?.content);
                // console.log(data);
              } catch (error) {
  ...
  return (
    ...
    ?
    <main className='relative mx-auto  w-[98%] min-[1256px]:w-[90%] '
    onMouseLeave={()=>{setShowArrows(false)}}
    onMouseEnter={()=>{setShowArrows(true)}}
    >
    <h2 className='bg-black text-white text-[18px] min-[600px]:text-[25px]  font-semibold pl-[10px] min-[600px]:pl-[15px] min-[768px]:pl-[40px] py-2 uppercase'>{singleCategory} TV SHOWS</h2>
